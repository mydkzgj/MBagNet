    ...
    def _update(engine, batch):
        ...
        if model.reloadState == True:
            ...
            if model.maskedImgReloadType == "seg_mask":
                ...
                soft_mask = model.lesionFusion(soft_mask, labels[labels.shape[0]-soft_mask.shape[0]:labels.shape[0]])
                max_kernel_size = random.randint(80, 320)
                soft_mask = torch.nn.functional.max_pool2d(soft_mask, kernel_size=max_kernel_size*2+1, stride=1, padding=max_kernel_size)
                soft_mask = torch.nn.functional.avg_pool2d(soft_mask, kernel_size=40, stride=1, padding=20)
        ...
        weight = {"cross_entropy_multilabel_loss":1, "cross_entropy_loss":1, "seg_mask_loss":1, "gcam_mask_loss":1, "pos_masked_img_loss":0.05, "neg_masked_img_loss":0.2, "for_show_loss":0}
        gl_weight = [1, 1, 1, 1]
        loss = 0
