...
class CJY_DUAL_BACKPROPAGATION():
    ...
    def conv_backward_hook_fn(self, module, grad_in, grad_out):
        if self.guidedCONVstate == True:
            ...
            if self.bias_back_type == 1:
                ...
                active_neuron = torch.nn.functional.conv_transpose2d(torch.ones_like(bias_overall), new_weight, stride=module.stride,
                                                                     padding=new_padding, output_padding=output_padding)
    ...
    def AvgPoolCAM(self):
        ...
        for i in range(len(self.target_layer)):
            ocam = torch.nn.functional.avg_pool2d(ocam, kernel_size=2, stride=2, padding=1)
