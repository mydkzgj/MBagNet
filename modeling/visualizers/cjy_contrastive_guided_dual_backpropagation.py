...
class CJY_CONTRASTIVE_GUIDED_DUAL_BACKPROPAGATION():
    def __init__(self, model, num_classes, target_layer, useGuidedBP=False):
        ...
        self.contrastive_guided_pgrad_cam.reservePos = True
    ...
    def GenerateVisualiztions(self, logits, labels, input_size, visual_num):
        ...
        dbcam_list, _, _ = self.dual_backpropagtion.GenerateVisualiztions(logits, labels, input_size, visual_num)
        cgpcam_list, _, _ = self.contrastive_guided_pgrad_cam.GenerateVisualiztions(logits, labels, input_size, visual_num)

        self.gcam_list = []
        self.gcam_max_list = []
        self.overall_gcam = None

        for i in range(self.target_layer):
            dbcam = dbcam_list[i]
            cgpcam = cgpcam_list[i]

            gcam = dbcam * cgpcam.gt(self.threshold)

            norm_gcam, gcam_max = self.gcamNormalization(gcam)
            # CJY at 2020.10.22
            self.gcam_list.append(norm_gcam)
            self.gcam_max_list.append(gcam_max)
