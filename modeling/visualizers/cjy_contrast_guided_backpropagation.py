    ...
    def relu_backward_hook_fn(self, module, grad_in, grad_out):
        if self.guidedReLUstate == True:
            ...
            if relu_output.ndimension() == 4:
                ...
                o_sum1 = torch.sum(relu_output1 * o_grad_input1, dim=1, keepdim=True).relu()
                o_sum2 = torch.sum(relu_output1 * o_grad_input2, dim=1, keepdim=True).relu()
                n_sum1 = torch.sum(relu_output1 * n_grad_input1, dim=1, keepdim=True)
                n_sum2 = torch.sum(relu_output1 * n_grad_input2, dim=1, keepdim=True)
