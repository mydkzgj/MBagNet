    ...
    def DrawVisualization(self, imgs, labels, plabels, gtmasks, threshold, savePath):
        ...
        for j in range(imgs.shape[0]):
            for i, gcam in enumerate(self.gcam_list):
                ...
                visual_prefix = layer_name.replace(".", "-") + "_L{}_P{}_S{}_".format(labels[j].item(), plabels[j].item(), self.observation_class[j])
                draw_visualization(imgs[j], gcam[j], gtmasks[j], threshold, savePath, str(self.draw_index), visual_prefix)
