...
class CJY_MWP_L2():
    ...
    def __init__(self, model, num_classes, target_layer):
        ...
        # 不同layer的导向guided方式，通用参数配置为
        # state=1 or 0 用于是否在反向传播中进入hook函数，因为hook设置后必须能区分正常反向传播以及修改的反向传播（结束后state置0）
        self.guidedReLUstate = 0

        self.useGuidedReLU = True
        self.num_relu_layers = 0
        self.relu_output = []
        self.relu_current_index = 0
        self.stem_relu_index_list = []

        self.useGuidedMAXPOOL = False
        ...
        self.useGuidedAdaptiveAVGPOOL = False
    ...
    def avgpool_backward_hook_fn(self, module, grad_in, grad_out):
        if self.guidedAVGPOOLstate == True:
            self.avgpool_input_obtain_index = self.avgpool_input_obtain_index - 1
            avgpool_input = self.avgpool_input[self.avgpool_input_obtain_index]
    ...
    def ObtainGradient(self, logits, labels):
        ...
        self.avgpool_input_obtain_index = len(self.avgpool_input)
