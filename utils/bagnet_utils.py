...
#CJY
def show(model, img, imgLabel, patchsize):

    mean = np.array([0.4914, 0.4822, 0.4465])
    var = np.array([0.2023, 0.1994, 0.2010])
    heatmap = generate_heatmap_pytorch(model, img, imgLabel, 9)
    # 选取heatmap中的部分点，事实证明原始模型决策采用部分位置的patch，而不是可视化时的所有patch
    """
    count = 0
    heatmap_copy = heatmap.copy()
    for i in range(heatmap_copy.shape[0]):
        for j in range(heatmap_copy.shape[1]):
            if (i - 4) % 8 == 0 and (j - 4) % 8 == 0 and i < 220 and j < 220:
                count = count + 1
            else:
                heatmap_copy[i][j] = 0    
    l = heatmap_copy.sum() / count
    """
    ...
    plt.axis('off')
    ax = plt.subplot(122)
    ax.set_title('heatmap'+"(label="+str(imgLabel)+")")
    plot_heatmap(heatmap, img, ax, dilation=0.5, percentile=99, alpha=.25)
    plt.axis('off')
    #plt.show()
    global  save_img_index
    plt.savefig("D:/Research/MIP/Experiment/MBagNet/work_space/heatmap/"+ "heatmap_"+str(save_img_index) + '_' + '.png', bbox_inches='tight', pad_inches=0, dpi=150)
